...
import org.srluren.web.manager.impl.CreditoPmygManager;
import org.srluren.web.resources.ResponsePage;

/**
 * Servlet implementation class GenCronoCapitalServlet
 */
public class GenCronoMicroServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GenCronoMicroServlet() {
        super();
	...
	private void procesar(HttpServletRequest request,
		...
		CreditoPmygManager creditoPYMGManagerImpl = new CreditoPmygManager();
		
		//Obtiene cronograma de credito
		CreditoCronogramaBO creditoCronogramaBO = creditoPYMGManagerImpl.generarCronograma(request.getParameter("jsonRequest"),null);
		List<Cuota> listaCuotasCredito =  creditoCronogramaBO.getListaCuotasCreditoBO();
		
		//Devuelve datos de entrada
		CreditoCabeceraBO creditoCabeceraBO = creditoPYMGManagerImpl.getDatosCabeceraCredito();
		...
		if (creditoCronogramaBO.isPeriodoGracia()){
			dispatcher = getServletContext().getRequestDispatcher(ResponsePage.PAGE_CRED_MICRO_GRACIA);
		}else{
			dispatcher = getServletContext().getRequestDispatcher(ResponsePage.PAGE_CRED_MICRO);
		}
