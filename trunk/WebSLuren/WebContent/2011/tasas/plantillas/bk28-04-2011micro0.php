...
if($diferencia==0){//si a la primera dif es 0 que haga la tabla final
	...
	?>
        ...
        <td align="center" class="borde_tabla_filas" width="85"> <?php 		//colum4
			...
			$seg_degr=$calculos->redondeado ($seg_degr,2);
			//echo $seg_degr;
			echo number_format($seg_degrt,2);
			$total_seguro_desgrav=$total_seguro_desgrav+$seg_degr;
			?></td>
...
$prorrateo=$calculos->redondeado ($prorrateo, 2);
	...
	//echo $saldo_cap.'----'.substr($saldo_cap, -4, 4).'<br />';

		$limite_numero = substr($saldo_cap, -2, 2);
		
		if($limite_numero=="+9")
		{
			$diferencia=0;
		}
//		echo $diferencia.'---'.strpos($diferencia, "+").'<br>';
		
//		$tp = strpos($diferencia, "+");
		
		
//		$tp = substr($diferencia, 0, 3);
...
//			$arr_amort_capital	
		...
		?>
			...
			$valores_a_sumar = $d_sum_ven + $arr_amort_capital[$i-1];
