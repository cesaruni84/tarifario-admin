...
<script>
	$(document).ready(function(){
		...
		var reglas = {  
			 ...
			 plazoPrestamo: {required:true, number:true, min:120, max: 240},
		...
		var mensajes = {  
			...
			plazoPrestamo:{number:"Plazo del préstamo inválido, solo debe ingresar números",min:"El plazo mínimo del préstamo es de 120 meses",max:"El plazo máximo del préstamo es de 240 meses"}, 
		...
		//Validación de % máximo de financiamiento - 90%
		$("#montoCredito").focusout(function(){
			if ($("#valorInmueble").val()!="" && $("#montoCredito").val()!="" ){
				...
				if (parseFloat(montoCredito_) > 0.90 * parseFloat(valorInmueble_)  ){
					alert("El monto máximo de financiamiento es del 90% del valor del inmueble. Ingrese un menor monto del crédito");
				}
			}
		});
				...
				success : function(response) {
					...
					if(codigo == 0){
						...
						$.ajax({
							type: 'get',
							data : "jsonRequest="+json,
							url : './validacionAmortizaHipoServlet',
							success : function(response_) {
								...
								if(codigo_ != 0){
									...
									window.location.href =  './genCronoHipoMivvServlet?jsonRequest='+ json;
								}
							},
							error : function(response) {
								alert(response);
