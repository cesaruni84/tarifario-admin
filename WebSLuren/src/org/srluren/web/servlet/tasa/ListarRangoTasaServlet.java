...
/**
 * Servlet implementation class ListarRangoTasaServlet
 */
public class ListarRangoTasaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ListarRangoTasaServlet() {
        super();
	...
	private void procesar(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			if(beanTasa != null){
				...
				responseJSON.put("msj_retorno", parameters.getKey("MSJ_VAL_TASA_OK") +  ": "+ TEA_MIN + "% - " + TEA_MAX + "%" );

			}else{
				responseJSON.put("cod_retorno", parameters.getKey("COD_VAL_TASA_OK"));
				responseJSON.put("msj_retorno", "");
			}
